...
Az igazság a QL -ben; a \emph{truth in a model}. A QL-ben található mondatok nem simán igazak vagy hamisak mint a szimpla szimbólumok, de csak a modellhez relatívak. A modell biztosítja a szimbólumok jelentését addig, amíg bármilyen különbséget teremt az igazság és a valótlanság felé.
...
\begin{quote}
Vegyünk néhány tetszőleges modellt; \model{M}. Ezután vegyünk egy tetszőleges szereplőt a UD-ból; kényelmünk érdekében, nevezzük ezt $\Omega$ -nak. Ez lehet az oka annak, hogy $\langle\Omega,\Omega\rangle$ a $R$ kiterjesztés része, vagy nem része. Amennyiben a $\langle\Omega,\Omega\rangle$ a $R$ kiterjesztés része, akkor a $Rxx$ elégedett egy változó feladattal, ami hozzárendeli a $\Omega$ a $x$ -hez (az 1. résztől); miután az ezt megelőző $Rxx\eif Rxx$ nem volt megfelelő,  a feltétel teljesül (az 5. résztől). Mindkét esetben, $Rxx\eif Rxx$ kielégítő, megvalósult. Ez az UD bármely szereplőjére igaz, tehát a $\forall x(Rxx \eif Rxx)$ megvalósul bármely igaz érték hozzárendelésnél (a 7. résztől). Tehát a $\forall x(Rxx \eif Rxx)$ igaz, a \model{M} -ben, (az igazság meghatározott jelentése esetében). Ez az érvelés kimutatja, hogy független a pontos UD-től, és független a pontos kiterjesztésétől a $R$-nek, tehát $\forall x(Rxx \eif Rxx)$ bármely modellben megvalósul. Ezek alapján tehát, ez egy tautológia.
...
Ahhoz, hogy minden lehetséges modellről érveket adjunk, két stratégia okos kombinálása szükséges:

1. Felosztani az eseteket a két lehetséges fajta/jelleg között, úgy, hogy minden esetnek az egyik vagy a másik fajtának/jellegnek kell lennie. A p.~\pageref{allmodels1}, -ről szóló érvelésben például kétféle modellt különböztettünk meg arra alapozva, hogy egy speciálisan hozzárendelt pár a  \extension{R}-ben volt e vagy sem. A fenti érvelésben, mi az eseteket az alapján különböztettük meg, hogy a hozzárendelt pár a \extension{R}-ben volt, vagy nem volt.

2. Tekintsünk egy korlátlan tárgyat aszerint, hogy valamilyen általános dolgot mutat. A fenti érvelésben fontos volt, hogy a $\Omega$ csak valamiféle korlátlan tagja volt az UD-nek. Mi nem feltételeztünk semmilyen speciális dolgot róla. Eszerint bármit, amit ki tudunk mutatni az $\Omega$ -nek, annak muszáj megragadnia az UD minden tagját – ha az $\Omega$ -nek sikerülne kimutatni, akkor bármi másnak is sikerülne. A hasonló esetben, mi nem feltételeztünk semmi rendkívülit a \model{M} -ről, szóval bármi mást is ki tudnánk mutatni a \model{M} -ből, és annak muszáj minden modellre érvényesnek lennie.
