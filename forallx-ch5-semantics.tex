...
Az igazság Ql-ben \emph{igazság a modellben).QL mondatai nem laposak- egyszerű szimbólumokként igazak vagy hamis hamisak, de csak a a modelben relativ. A modell adja a szimbolumok jelentését,  amennyiben ez tesz bármennyi különbséet az igazságban és a hamisságban.
...
\Alosztály{Ervelés az összes modellben (visszatérés)}
A bekezdés végén \ref{sec.HasználtModellek}, Akadályba ütköztünk amikor megpróbáltuk megmutatni ezt $\forall x(Rxx\eif Rxx)$ ez egy felesleges szószaporitás. Meghatározott elégedettségünkben most igy tudunk érvelni:
\begin{quote}
Megfontol néhány önkényes modellt \model{M}. Most megfontoljuk az UD egy önkényes tagját; kényelem kedvéért nevezzük $\Omega$ -nak. Ennek igy kell lennie bármely esetben $\langle\Omega,\Omega\rangle$ Ha a kiterjesztésben van akkor $R$ vagy ez nem. Ha $\langle\Omega,\Omega\rangle$ benne van a $R$ kiterjesztésben, akkor $Rxx$ elégedett egy változó kinevezésével amely $\Omega$ to $x$ (az 1. részben definiált {megelégedés}); Azóta a követekzetes $Rxx\eif Rxx$ megelégedett, a kondició is megelégedett (5-ös rész) Ha $\langle\Omega,\Omega\rangle$ nem a $R$ kiterjesztésében van, akkor $Rxx$ nem megelégedett a változó kinevezéssel, amit az kijelöl $\Omega$ to $x$ (1. rész); Az előzménye óta $Rxx\eif Rxx$ nem megelégedett, a köndiciója megelégedett(5. rész). Ebben az esetben, $Rxx\eif Rxx$ megelégedett. Ez igaz az összes UD tagra, szóval $\forall x(Rxx \eif Rxx)$ megelégedett bármely igaz értékkel (7. rész). Szóval $\forall x(Rxx \eif Rxx)$ igaz a \model{M} ben. (A definició miatt {igaz}).  Ez az érv a pontos UDra való tekintet nélkül és a pontos kiterjesztésre való tekintet nélkül tart a $R$, szóval $\forall x(Rxx \eif Rxx)$ igaz bármely modellben. Ezért ez egy tautológia.
\end{quote}


Giving arguments about all possible models typically requires clever combination of two strategies:

Az összes lehetséges modellről való vitához szükség van a 2 stratégia okos kombinálására.
...
Kész lehetséges fajta lehet, mint például minden esetnek kell egy fajtának lennie vagy másnak. Az érrvelésben p.~\pageref{allmodels1}, például kettőt különböztetünk meg, modell alapút és nem speciális rendezett párokat. Ilyen volt a \extension{R} ben. Az érvelésben fentebb, olyan lehetőségeket különböztettünk meg, amelyekben rendezett párok voltak a \extension{R}-ben és olyan esetekben melyen nem voltak azok.
...
Bemutatunk egy önkényes objektumot sokkal általánosabban. A fenti érvben kulcsfontosságú volt az, hogy $\Omega$ csak egy önkányes tagja volt UD-nek. Mi nem feltételeztünk róla semmi különlegeset. Ahogy ilyen, bármit meg tudunk mutatni és megtartjuk a $\Omega$ és meg kell tartani az összes UD--- tagot. Ha meg tudjuk mutatni $\Omega$ -ra, akkor be tudjuk mutatni bármire. Ugyanigy, mi nem feltételeztünk semmi különlegeset \model{M}-ről, és bármit bemutathatunk \model{M}-ről minden modellre kell vonatkoznia. 
%ML fordítása vége
